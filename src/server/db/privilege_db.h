...
const TQueries Queries =
		{ { EGetAppPermissions,
				"SELECT * FROM app_permission_view WHERE app_name='%Q'" },
				{ EAddAppPermissions,
						"INSERT INTO app_permission_view (app_name, pkg_name, permission_name) VALUES (?, ?, ?)" },
				{ ERemoveAppPermissions,
						"DELETE FROM app_permission_view WHERE app_name=? AND pkg_name=? AND permission_name=?" },
				{ EAppIdExists, "SELECT * FROM app WHERE name=?" }, {
						EPkgIdExists, "SELECT * FROM pkg WHERE name=?" },
				{ EPermissionForPkgIdExists,
						"SELECT * FROM app_permission_view WHERE pkg_name=? AND permission_name=?" },
				{ EBeginTransaction, "BEGIN;" },
				{ ECommitTransaction, "COMMIT;" }, { ERollbackTransaction,
						"ROLLBACK;" } };
...
public:
	 ...
	 * @return TResult::EOperationSuccessful on success, TResult::EOperationFailed on failure
	 */
	TResult PkgIdHasPermissions(const char * const pkgId,
	 ...
	 * @return TResult::EOperationSuccessful on success, TResult::EOperationFailed on failure
	 */
	TResult AddPermissions(const char * const appId, const char * const pkgId,
			TPermissionsList &permissions, TPermissionsList *newPermissions);
	 ...
	 * @return TResult::EOperationSuccessful on success, TResult::EOperationFailed on failure
	 */
	TResult RemovePermissions(const char * const appId,
