...
/*
 #undef LogPedantic
 #define LogPedantic(msg) std::ostringstream log;     \
        log << msg;                                  \
        printf ("%s, %s, %d, %s\n",					 \
            log.str().c_str(),                       \
...
TResult PrivilegeDb::PkgIdHasPermissions(const char *pkgId,
		TPermissionsList &permissions) {

	TPermissionsList tmpList(permissions);

	try {
		SecurityServer::DB::SqlConnection::DataCommandAutoPtr command =
				mSqlConnection->PrepareDataCommand(
						Queries.at(TQueryType::EPermissionForPkgIdExists));
		for (TPermissionsList::iterator plIter = tmpList.begin();
				plIter != tmpList.end(); ++plIter) {
			command->BindString(1, pkgId);
...
TResult PrivilegeDb::AddPermissions(const char *appId, const char *pkgId,
		TPermissionsList &permissions) {

	TPermissionsList tmpList(permissions);
	TResult ret = PrivilegeDb::PkgIdHasPermissions(pkgId, permissions);
...
TResult PrivilegeDb::RemovePermissions(const char *appId, const char *pkgId,
		TPermissionsList &permissions) {
