...
typedef std::unique_ptr<FTS, std::function<void(FTS*)> > FTSUniquePtr;



/// Label used for publicily shared directories
#define LABEL_FOR_PRIVATE_DIRS	"User"
...
static int dir_set_smack(const char *path, const char* label,
...
static bool loc_label_dir(std::string pathString, std::string labelString,
...
bool InstallerService::setupPath(std::string pkgId, std::pair<std::string, int>appPath)
{
	...
	case SECURITY_MANAGER_PATH_PRIVATE:
		label.assign(LABEL_FOR_PRIVATE_DIRS);
	...
	default:
		...
		goto setupPath_error;
		break;
	}
	if (!loc_label_dir(path, label, label_transmute, label_executables))
			return false;
	else return true;
