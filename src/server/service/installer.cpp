...
static int labelAll(const FTSENT *ftsent __attribute__((unused)))
{
...
static int labelDirs(const FTSENT *ftsent)
{
    LogSecureDebug("Entering function: " << __func__);
...
static int labelExecs(const FTSENT *ftsent)
{
    LogSecureDebug("Entering function: " << __func__);
...
static int labelLinksToExecs(const FTSENT *ftsent)
{
    LogSecureDebug("Entering function: " << __func__);
        ...
        const char *xattr_name, label_decision_fn fn)
{
    ...
    FTSENT *ftsent;
    int ret;
    int len = strnlen(label.c_str(), SMACK_LABEL_LEN + 1);
    ...
    while ((ftsent = fts_read(fts.get())) != NULL) {
        ...
        if (ret == DECISION_LABEL) {
            if (lsetxattr(ftsent->fts_path, xattr_name, label.c_str(), len, 0) != 0) {
        ...
        bool set_transmutable, bool set_executables)
{
    ...
    ret = dirSetSmack(path.c_str(), label.c_str(), XATTR_NAME_SMACK, labelAll);
    if (true != ret) {
    ...
    if (set_transmutable) {
        ...
        ret = dirSetSmack(path.c_str(), "TRUE", XATTR_NAME_SMACKTRANSMUTE, labelDirs);
        if (true != ret) {
    ...
    if (set_executables) {
        ret = dirSetSmack(path.c_str(), label.c_str(), XATTR_NAME_SMACKEXEC, &labelExecs);
        if (true != ret)
        {
        ...
        ret = dirSetSmack(path.c_str(), label.c_str(), XATTR_NAME_TIZENEXEC, &labelLinksToExecs);
        if (true != ret)
        {
