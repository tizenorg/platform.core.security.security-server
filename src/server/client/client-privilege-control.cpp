...
#define CHECK_STRING(str) if ((nullptr == str) || (0 == strlen(str))) {   \
                              LogError(#str " is nullptr or empty");          \
                              return PC_ERR_INVALID_PARAM;                 \
                           }

/*
 * Verifies if an argument is not nullptr.
 * Used is most of functions below.
 */
#define CHECK_NOT_NULL(var) if (nullptr == var) {            \
                                LogError(#var " is nullptr");    \
                                return PC_ERR_INVALID_PARAM;  \
                             }

/*
 * Convert std::vector<std::string> to null-terminated array of null-terminated c-strings
 */
int stringsVectorToArrayOfCStrings(const std::vector<std::string>& input, char*** output)
{
    // +1 for nullptr at the end
    ...
    size_t i = 0;
    for (; i < input.size(); ++i) {
        if (nullptr == ((*output)[i] = strdup(input[i].c_str()))) {
            goto fail;
        }
    }

    (*output)[input.size()] = nullptr;
...
char* security_server_app_id_from_socket(int sockfd)
{
    ...
    try {
        ...
        makeHeader(send, LibprivilegeControlAction::APP_ID_FROM_SOCKET);
...
int security_server_app_revoke_permissions(const char* pkg_id)
{
    ...
    return try_catch([&] {
        ...
        //put arguments into buffer
        makeHeader(send, LibprivilegeControlAction::APP_REVOKE_PERMISSION);
        Serialization::Serialize(send, std::string(pkg_id));
...
int security_server_app_add_friend(const char* pkg_id1, const char* pkg_id2)
{
    ...
    return try_catch([&] {
        ...
        //put arguments into buffer
        makeHeader(send, LibprivilegeControlAction::APP_ADD_FRIEND);
...
SECURITY_SERVER_API
int security_server_app_enable_permissions(const char *pkg_id, app_type_t app_type,
...
SECURITY_SERVER_API
int security_server_app_disable_permissions(const char *pkg_id, app_type_t app_type,
